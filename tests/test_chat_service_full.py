...
async def test_execute_tool_exception():
    ...
    with patch("solomon.services.chat_service.mcp_client_mod.call_tool", side_effect=RuntimeError("Connection failed")):
...
async def test_execute_tool_success():
    ...
    with patch("solomon.services.chat_service.mcp_client_mod.call_tool", new_callable=AsyncMock) as mock_call:
...
async def test_complete_chat_mocked(db_session, app_config):
    ...
    with (
        patch("solomon.services.chat_service.llm_client.chat_completion", new_callable=AsyncMock, return_value=mock_llm_result),
        patch("solomon.services.chat_service.prompt_builder.build_system_prompt", new_callable=AsyncMock, return_value=("System prompt", [])),
        patch("solomon.services.chat_service.prompt_builder.append_mcp_context", new_callable=AsyncMock, return_value="System prompt"),
    ):
